class Shepherd {
  constructor(x, y, herd, brain) {
    ...
    if (brain) {
      ...
      let inputSize = 2 + this.herd.sheep.length * 2; // x,y for each sheep
      this.brain = new NeuralNetwork(inputSize, 32, 1);
    }
  ...
  run() {
    ...
    if (!sys.nnControl) { // training mode
  ...
  think(targetArray) {
    ...
    if (sys.nnControl) {
      return this.brain.predict(inputArray);
