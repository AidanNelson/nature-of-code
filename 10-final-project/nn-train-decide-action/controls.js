...
function setupSliders() {
  ...
  function systemControls() {
    this.nnControl = false;
    this.saveDog = function() {
      let data = dog.brain.serialize();
      let name = dog.brainTrainings + '.json';
      save(data, name);
    };
    this.loadDog = function() {
...
function jsonLoaded(result) {
  let newBrain = NeuralNetwork.deserialize(result);
  dog = new Shepherd(10, 10, herd, newBrain);
}